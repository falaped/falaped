 ...
 * fica `paid`. Idempotente se já foi assumido pelo mesmo perfil.
 */
export async function claimLeadBook(supabase: SupabaseClient, bookId: string, profileId: string): Promise<void> {
  const { data: book, error } = await supabase.from("books").select("id, lead_id, profile_id").eq("id", bookId).maybeSingle()
  ...
  const { error: bookError } = await supabase.from("books").update({ profile_id: profileId, updated_at: now }).eq("id", bookId)
