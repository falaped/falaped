...
test("deleteMedicalCertificatesBulk makes exactly one DB call and one storage call for 10 ids", async () => {
  let dbCallCount = 0
  ...
  await deleteMedicalCertificatesBulk(supabase, ids, "profile-id", paths)
  assert.equal(dbCallCount, 1)
  assert.equal(storageCallCount, 1)
})

// Empty array: returns { deletedCount: 0 } and makes zero storage calls
test("deleteMedicalCertificatesBulk returns { deletedCount: 0 } and makes zero calls for empty input", async () => {
  let dbCallCount = 0
  let storageCallCount = 0
  const supabase = makeSpySupabase({
    onDbCall: () => { dbCallCount++ },
  ...
  const result = await deleteMedicalCertificatesBulk(supabase, [], "profile-id", [])
