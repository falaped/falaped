...
test("deleteMedicalReport does not throw when profileId does not match (ownership no-op)", async () => {
  const supabase = makeMockSupabase({ dbError: null })
  await assert.doesNotReject(() =>
    deleteMedicalReport(supabase, "some-uuid", "other-profile-id", null),
  )
})

test("deleteMedicalReport deletes own report successfully", async () => {
  const supabase = makeMockSupabase()
  await assert.doesNotReject(() =>
    deleteMedicalReport(supabase, "own-uuid", "own-profile-id", null),
  )
})

// Storage error must NOT reject — DB row already gone; orphan PDF is preferable to IDOR
test("deleteMedicalReport logs storage error but does not throw", async () => {
  const supabase = makeMockSupabase({
