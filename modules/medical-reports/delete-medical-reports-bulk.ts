...
/**
 * Deletes multiple medical reports in a single batched DB operation, ownership-scoped (D-15).
 ...
 */
export async function deleteMedicalReportsBulk(
  ...
  if (error) {
    throw new Error(`[MEDICAL_REPORTS] Bulk delete failed: ${error.message}`)
  }
  ...
  if (paths.length > 0) {
    ...
    if (storageError) {
      console.error(
        `[MEDICAL_REPORTS] Orphan PDFs not removed after bulk delete: ${storageError.message}`,
      )
    }
  }
