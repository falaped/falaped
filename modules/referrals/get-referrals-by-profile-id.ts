...
/**
 * Fetches referrals for a profile (doctor), ordered by issued_at desc.
 * patient_name is taken from payload.patientName for list display.
 */
export async function getReferralsByProfileId(
  ...
  if (error) {
    throw new Error(`[REFERRALS] Failed to fetch list: ${error.message}`)
  }
