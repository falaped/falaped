...
/**
 * Deletes multiple exam requests in a single batched DB operation, ownership-scoped (D-15).
 ...
 */
export async function deleteExamRequestsBulk(
  ...
  if (error) {
    throw new Error(`[EXAM_REQUESTS] Bulk delete failed: ${error.message}`)
  }
  ...
  if (paths.length > 0) {
    ...
    if (storageError) {
      console.error(
        `[EXAM_REQUESTS] Orphan PDFs not removed after bulk delete: ${storageError.message}`,
      )
    }
  }
