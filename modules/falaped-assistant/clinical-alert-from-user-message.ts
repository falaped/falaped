import type { ClinicalAlertItem } from "@/modules/falaped-assistant/contracts/route-result"
...
export function hasExplicitGuardianQuotedOrShoutSignal(message: string): boolean {
  ...
  const hasDirectGuardianCue =
    ...
    /\b(mae|pai|responsavel|genitora|genitor)\s+(disse|relata|conta|informa|avisou)\b/.test(normalized) ||
...
const SIGNAL_RULES: SignalRule[] = [
  ...
  },
  {
    ...
    matches: (n) =>
      /\b(tiragem|retracao\s+intercostal|retracao\s+subcostal|retracao\s+supraclavicular)\b/.test(n) ||
      /\b(retracao|batimento\s+de\s+narina|batimento\s+de\s+asa\s+de\s+nariz|gemencia)\b/.test(n),
  },
  {
  ...
  },
  {
    ...
    matches: (n) =>
      /\b(oxigenio|oxigenoterapia|o2\s+suplement|em\s+o2|com\s+o2|sobre\s+o2|cateter\s+nasa|mascara)\b/.test(n),
  },
  {
    id: "cyanosis",
  ...
  },
  {
    ...
    matches: (n) =>
      /\b(choque|hipotensao|perfusao\s+(?:comprometida|diminuida)|tempo\s+de\s+enchimento|pulsos\s+finos)\b/.test(n),
  },
  {
