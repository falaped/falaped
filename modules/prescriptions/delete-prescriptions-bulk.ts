...
/**
 * Deletes multiple prescriptions in a single batched DB operation, ownership-scoped (SEC-01).
...
export async function deletePrescriptionsBulk(
  ...
  if (paths.length > 0) {
    ...
    if (storageError) {
      console.error(
        `[PRESCRIPTIONS] Orphan PDFs not removed after bulk delete: ${storageError.message}`,
      )
    }
  }
